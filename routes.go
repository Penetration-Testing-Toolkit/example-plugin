...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/Penetration-Testing-Toolkit/example-plugin/internal/template"
...
const cssPath string = "/static/css/output.css"
...
func (m *ModuleExample) setupRoutes() {
	...
	router.routes[r.Method][rootPath+r.Path] = m.css
	info.Routes = append(info.Routes, r)
...
// css "GET /static/css/output.css" returns the plugin's CSS file.
// Note that "Content-Type" must be "text/css" in order for PTT to handle it properly.
func (m *ModuleExample) css(_ context.Context, _ *http.Request) (*shared.Response, error) {
	return &shared.Response{
